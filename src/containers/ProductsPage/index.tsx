...
const ProductsPage: React.FC = () => {
    ...
    const [isSearchRequestFired, setIsSearchRequestFired] = useState<Boolean>(false);
    const [isGetApiDataFailed, setIsGetApiDataFailed] = useState<Boolean>(false);
    const [searchParams, setSearchParams] = useState<SearchParam>({searchText: '', page: 0, totalCount: 0});
     ...
     * @param isLoadMore
     */
    function getApiData(queryParam: string, page: number = 0, isLoadMore: boolean = false): void {
        fetch('https://latency-dsn.algolia.net/1/indexes/*/queries', {
            ...
            body: JSON.stringify(
                {requests:[{'indexName':'ikea','params':`query=${queryParam}&hitsPerPage=16&page=${page}`}]})
        })
